...
## 11. 🛡️ Blindaje de Entorno Local, Limpieza SQLite & Protocolo Karpathy
- **Aislamiento Estricto Local vs. Cloud en `/inicia-sesion`**:
  - Se modificó la regla del comando `/inicia-sesion` (`.opencode/commands/inicia-sesion.md` y `.agents/AGENTS.md`) para ejecutar estrictamente `supabase start` con datos de prueba locales (`seed.sql`).
  - Se eliminó la dependencia de `npm run db:sync` en el arranque, asegurando que el desarrollo local sea 100% offline y que la base de datos de producción/cloud solo se consulte con el comando explícito `/actualiza-datos`.
- **Corrección de Esquemas PostgREST en Supabase Local (`config.toml`)**:
  - Se limitó `schemas = ["public", "graphql_public"]`, eliminando referencias a esquemas no migrados (`agent_memory`) que causaban errores `503 / unhealthy` en PostgREST al arrancar.
- **Mantenimiento y Compactación de Base de Datos OpenCode (`opencode.db`)**:
  - Se optimizó y redujo la base de datos de OpenCode de **3.8 GB → 20 MB** eliminando registros huérfanos y compactando con `VACUUM`.
  - Creado el script de mantenimiento automatizado `tools/opencode-vacuum.sh`.
- **Corrección de Build y Tipos en Frontend**:
  - Corregida la ruta de importación `@/lib/supabase/server` en `src/app/api/reminders/route.ts`.
  - Instaladas dependencias y componentes faltantes (`swr`, `skeleton`).
  - Eliminada la consulta a la columna inexistente `is_active` en `src/lib/billing/utils.ts`.
  - Build de Next.js verificado y compilando al 100% (`38/38` rutas generadas exitosamente).
