  ...
  return (
    <div className="relative">
      <Button
        ...
        className="rounded-xl text-muted-foreground hover:text-foreground hover:bg-muted h-10 w-10 relative cursor-pointer"
      >
      ...
      {isOpen && (
        <>
          ...
          {/* Popover */}
          <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-card rounded-2xl border border-border shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="max-h-80 overflow-y-auto divide-y divide-border/60">
              {notifications.length === 0 ? (
                ...
                notifications.map((n) => (
                  <div
                    key={n.id}
                    className={`p-4 transition-colors ${!n.read ? "bg-success/5" : "hover:bg-muted"}`}
                  >
