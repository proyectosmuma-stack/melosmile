...
export default function BillingHubPage() {
  ...
  const getStatusBadge = (status?: string) => {
    switch (status) {
      case "approved":
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 border border-emerald-300">🟢 Aprobado</span>;
      case "pending_review":
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-800 border border-amber-300">🟡 En Revisión</span>;
      case "invoiced":
        return <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 border border-blue-300">🟦 Facturado</span>;
      case "draft":
    ...
    <div className="min-h-screen bg-slate-50 p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <div className="text-xs font-medium text-slate-500 uppercase">NETO Aprobado ({selectedYear})</div>
            <div className="text-2xl font-extrabold text-slate-900">{totalNetoAño.toLocaleString("es-ES", { minimumFractionDigits: 2 })} €</div>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <div className="text-xs font-medium text-slate-500 uppercase">Pendientes de Aprobación</div>
            <div className="text-2xl font-extrabold text-slate-900">{pendingSessions.length} meses</div>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <div className="text-xs font-medium text-slate-500 uppercase">Clínicas Activas</div>
            <div className="text-2xl font-extrabold text-slate-900">{clinics.length} sedes</div>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <div className="text-xs font-medium text-slate-500 uppercase">Meses Completados</div>
            <div className="text-2xl font-extrabold text-slate-900">{approvedSessions.length} meses</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 px-3 py-1 bg-slate-100 rounded-lg text-sm font-bold text-slate-800">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-sm text-slate-600">
            ...
            <select
              ...
              className="bg-slate-50 border border-slate-300 rounded-lg px-3 py-1.5 text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500"
            >
                ...
                <div className="p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  ...
                  return session ? (
                    <Link key={monthNum} href={`/billing/${session.id}`}>
                      <div className="group relative bg-white hover:bg-slate-50 p-4 rounded-xl border border-slate-200 hover:border-emerald-500 shadow-xs hover:shadow-md transition-all cursor-pointer flex flex-col justify-between h-36">
                        <div>
                        ...
                        <div className="border-t border-slate-100 pt-2">
                          <div className="text-xs text-slate-400 font-medium">NETO acumulado</div>
