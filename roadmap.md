...
- ✅ **Fix Musly: cambio de clínica en citas CONFIRMADO E2E (2026-09-14):** `bulk_reschedule` ahora persiste la clínica **destino** (`clinic` + `source_clinic`), permite misma fecha, y la rama `update` resuelve clínica por nombre (commit `e2ac083`, **pendiente push/merge a `main`**). Desplegado a `agenda.melosmile.com` y verificado: revert de 4 citas a Daniel Bustamante → Musly (n8n ejecución `4919`) las movió a Goya → Supabase producción confirmó 6/6 citas del 22/09 en Goya.
...
- [ ] **Configurar Credenciales de Producción de Odoo**: Cargar en Vercel `melosmile-production` las credenciales finales de Odoo (`ODOO_URL`, `ODOO_DB`, `ODOO_USER`, `ODOO_PASSWORD`/`ODOO_API_KEY`) para emisión y sincronización contable real.
